export default {
  async fetch(request, env, ctx) {
    ...
    // Verify timestamp (24 hours expiration - much better UX)
    const now = Date.now();
    ...
    // Verify signature (using NEW algorithm that matches frontend)
    ...
    // Enhanced security checks for longer expiration
    ...
    // Basic rate limiting per device (since URLs last longer)
    const rateLimitKey = `rate_limit:${deviceFingerprint || 'unknown'}`;
    const currentHour = Math.floor(now / (60 * 60 * 1000)); // Current hour timestamp
    
    // Get rate limit data from KV (if you have KV storage)
    // For now, we'll use a simple in-memory approach (resets on worker restart)
    const rateLimitData = this.getRateLimitData(rateLimitKey, currentHour);
    
    // Allow 100 requests per device per hour (adjust as needed)
    ...
    console.log('🔐 Request security info:', {
      ...
      urlExpiresIn: Math.floor((expirationTime - (now - requestTime)) / (60 * 1000)) + ' minutes'
    });
  ...
  // NEW: Generate signature using the same algorithm as frontend
  ...
  generateSignatureSync(path, timestamp, secret) {
    const message = `${secret}:${timestamp}:${path}`;
    // Simple hash - old implementation
    let hash = 0;
    for (let i = 0; i < message.length; i++) {
      const char = message.charCodeAt(i);
